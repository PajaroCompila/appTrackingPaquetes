...
export class AutenticacionService {
  ...
  public iniciarSesion(nombreUsuario: string, contrasena: string) { return this.http.post<RespuestaSesion>(`${environment.urlApi}/autenticacion/iniciar-sesion`, { nombreUsuario, contrasena }).pipe(tap(({ usuario }) => { limpiarFiltrosSesion(); this.filtrosGlobales.reiniciar(); this.notificaciones.reiniciarSesion(); this.usuario.set(usuario); })); }
  ...
  public cerrarSesion() { return this.http.post<void>(`${environment.urlApi}/autenticacion/cerrar-sesion`, {}).pipe(tap(() => { limpiarFiltrosSesion(); this.notificaciones.reiniciarSesion(); this.usuario.set(null); })); }
